...
-record( load_profile, { owner                    % a PoD_ID or the name of some statistical sample
											 , interval_seconds = 3600  % default number of seconds in the intervals
...
%% Exported Functions
%%
-export([ new/1, put_hour/4, get_hour/3, put_day/3, map_profile_for_day_of_week_matching/2 ]).
...
put_hour( Profile, DayOfWeek, Hour, Usage ) ->
		...
		NewProfile.
get_hour( Profile, 'Monday', Hour )    -> get_hour( Profile, 1, Hour );
get_hour( Profile, 'Tuesday', Hour )   -> get_hour( Profile, 2, Hour );
get_hour( Profile, 'Wednesday', Hour ) -> get_hour( Profile, 3, Hour );
get_hour( Profile, 'Thursday', Hour )  -> get_hour( Profile, 4, Hour );
get_hour( Profile, 'Friday', Hour )    -> get_hour( Profile, 5, Hour );
get_hour( Profile, 'Saturday', Hour )  -> get_hour( Profile, 6, Hour );
get_hour( Profile, 'Sunday', Hour )    -> get_hour( Profile, 7, Hour );

get_hour( Profile, DayOfWeek, Hour ) ->
		Usage = orddict:fetch({ DayOfWeek, Hour*100 }, Profile#load_profile.hours ).		
...
make_Monday_profile_for_test() ->
		...
		Usage = [ 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700
						...
						, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300 ],  % Not a realistic profile -- just for test
		Profile1 = put_day( Profile0, 'Monday', Usage ),
		Profile1.		
...
get_hour_test() ->
    ...
    DayOfWeek = 'Sunday',
		Hour = 01,
		Usage = get_hour( Profile, DayOfWeek, Hour ),
		?assertEqual( 3100, Usage ).
