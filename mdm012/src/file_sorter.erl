...
%%
%% %CopyrightEnd%
%%% Modified by Richard Alexander Green July 2011 -- Add data to bad_object returns to aid debugging
%%
-module(file_sorter).
    ...
    case read(Fd, FileName, NoBytes, W) of
        {ok, Bin} ->
            ...
            case catch file_loop(L, I, Bin0, Bin, Size0, BSz0, BSz, Min, W)
                of
                {'EXIT', _R} ->
                    error({error, {bad_object, {'EXIT from file_loop', _R}, FileName}}, W);
                Reply ->
                    {Reply, NLSz}
            end;
...
fun_objs(Objs, L, LSz, NoBytes, I, W) ->
    ...
    case catch fun_loop(Objs, L, LSz, NoBytes, I, Keypos, Format, HL) of
        {'EXIT', _R} ->
            error({error, bad_object, {'EXIT from fun_loop', _R}}, W);
        Reply ->
            Reply
    end.
...
read(Fd, FileName, N, W) ->
    case file:read(Fd, N) of
        ...
        {error, enomem} ->
            %% Bad N
            error({error, {bad_object, 'enomem from file:read', FileName}}, W);
        {error, einval} ->
            %% Bad N
            error({error, {bad_object, 'einval from file:read', FileName}}, W);
        Error ->
            file_error(FileName, Error, W)
    end.
