...
install_transformers( N_Transformers, Transformer_Fanout, Circuit ) ->
		Transformer = object_factory:transformer_on_circuit( Circuit ), %assets:store( Transformer ),
...
install_services( N_Services, Transformer ) ->
		Service = object_factory:service_on_transformer( Transformer ), %assets:store( Service ),
		
		%?debugVal( Service ),
		Meter = object_factory:meter_on_service( Service ),	%assets:store( Meter ),
...
simulate_read_and_install( ThisDate, StopDate ) ->
		...
		Circuit = object_factory:make( circuit ), %assets:store( Circuit ),
...
simulate_days_demand( ThisDate ) ->				
		...
		MapFunction = fun(Meter) -> 
													...
													?assert( ServiceID =/= undefined ),    % This should not happen if install works correctly
...
send_days_demand( ServiceProfile, ThisDate ) ->
		...
		PoD_Series = #pod_series{ pod_ID = ServiceProfile#service_profile.service_ID
													  , series = Series },
		%gen_server:cast( {global, services}, PoD_Series ). % Insert this day`s demand into service history.
    services:do( PoD_Series ).
...
%see_if_delay_has_any_affect_on_following_test() ->
%		timer:sleep( 4000 ).
% No affect: Bug is still seen.

simulate_days_demand_test() ->
		?debugMsg("................. simulate_days_demand_test ............... simulate_days_demand_test "),
		ThisDate = {2011,04,24}, 
		simulate_days_demand( ThisDate ).
...
week_load_tester() ->
		?debugMsg("................. week_load_tester ................. week_load_tester "),
		
		% Give prior transactions time to clear
		?debugMsg("................. Give prior transactions time to clear  "),
		timer:sleep( 7*3000*2 ),
		
		mnesia:clear_table( object ),
		mnesia:clear_table( service_series ),
		mnesia:clear_table( transformer_series ),
		mnesia:clear_table( circuit_series ),
		
		% Simulate a week of meter reading and installation.
		?debugMsg("................. Simulate a week of meter reading and installation.  "),
		...
		?debugMsg("................. Give prior transactions time to clear  "),
		timer:sleep( 7*3000*2 ),
		NCircuits = mnesia:table_info( circuit_series,  size ),
		?assertEqual( 6, NCircuits ).

month_load_tester() ->
		?debugMsg("................. month_load_tester ................. month_load_tester "),
		...
		simulate_read_and_install( StartDate, StopDate ),
		?debugMsg("................. Give prior transactions time to clear  "),
		timer:sleep( 30*3000*2 ),
		
		mnesia:wait_for_tables([object,service_series,transformer_series,circuit_series], infinity ).
