...
%% - NOTE: Also stores those objects in the asset database.
...
record_asset( Asset ) -> 
		%?debugVal( { record_asset, Asset } ),	
		%gen_server:call( {global, assets}, Asset ),
		assets:store( Asset ),
		Asset.
...
make_id_test()          -> 
		?debugVal( mnesia:table_info( object_table, size ) ),	
		make( id, site ).
