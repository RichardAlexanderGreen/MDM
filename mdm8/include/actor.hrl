...
%% Created: Jan 6, 2011 -- Modified: June 12, 2011
%% Description: actor.hrl provides the Erlang wrapper
%%  that every business process actor will need.
%%  The actor definition adds do/2 and answer/1 functions 
%%  to handle asynchronous events, requests, and synchronous queries.
%% "Event" is this context will be implemented by the messenger module.
%% *** NOTE: THIS TEMPLATE ASSUMES THAT AN ACTOR IS SERVER-LIKE ::: RESEMBLING AN SOA SERVICE

-behaviour( gen_server ).    % TODO: ???? NOTE: There may be multiple instances of an actor.
...
log( Level, Report ) ->
		...
		% TODO: Might want to do this via messenger to a logger actor.
		ok.
...
log_open( LogName ) ->
		...
		put( log, IO_Device ),                             % TODO: This will not work for multiple instances of an actor.
		ok.
