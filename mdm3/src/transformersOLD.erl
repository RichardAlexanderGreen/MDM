...
?debugMsg( init ),
		...
		open_table( transformer_usage, "transformer_usage_table", [named_table, bag, {keypos,2} ] ),
...
usage( TransformerUsage = #pod_usage{} ) ->
		%?debugVal( TransformerUsage ),
		% Record the usage in the transformer's history
		Result = ets:insert( transformer_usage, TransformerUsage ),
		%{transformer, TransformerID} = TransformerUsage#pod_usage.pod_ID,
		TransformerID = TransformerUsage#pod_usage.pod_ID,
		[TransformerRec] = assets:get( transformer, TransformerID ),
		%?debugVal( TransformerRec ),
		CircuitID = TransformerRec#transformer.circuit_ID,
		%?debugVal( CircuitID ),
		Usage = TransformerUsage#pod_usage.usage,
		send_usage_to_circuit( Usage, CircuitID ).

send_usage_to_circuit( Usage, CircuitID ) ->
		%?debugVal( {send_usage_to_cirucit, Usage, CircuitID } ),
