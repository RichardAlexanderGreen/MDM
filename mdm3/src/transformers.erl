...
init([]) ->
		...
		open_table( transformer_series, "transformer_series_table", [named_table, set, {keypos,2} ] ),
...
add_series_into_PoD_history( PoD_Series ) ->
		...
		Transformer_PoD_Series = 
				        case ets:lookup( transformer_series, TransformerID ) of 
		...
		ets:insert( transformer_series, Updated_PoD_Series ),
...
flush() ->
		ets:tab2file( transformer_series, "transformer_series_table", [ {extended_info, [object_count] } ]  ),
